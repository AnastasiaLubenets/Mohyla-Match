...
begin
  ...
  if cardinality(normalized_looking_for_skill_ids) < 1 then
    raise exception 'At least one looking-for skill is required.'
      using errcode = 'P0001';
  end if;
...
grant select, insert, delete on public.blocks
  to service_role;
...
comment on function public.update_my_profile(
  ...
  'Atomically updates the authenticated user''s completed active profile and replaces required profile taxonomy relations. Runs as security invoker so RLS remains the authorization boundary.';
